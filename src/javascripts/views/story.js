...
//    element.append("<h4>Greenhouse gases</h4>");
 ...
 //   this.stories_for_choices(element, null, 22, 15);
  ...
  this.electricity_generation_capacity_table = function(element) {
    ...
    for (_i = 0, _len = values.length; _i < _len; _i++) {
      value = values[_i];
      if ((value.d2015 + value.d2050) !== 0.0) {
        html.push("<tr><td>" + value.name + "</td><td class='target'>" + (Math.round(value.d2015)) + "</td><td class='target'>" + (Math.round(value.d2050)) + "</td></tr>");
      }
    }
    html.push("</table>");
    element.append(html.join(''));
